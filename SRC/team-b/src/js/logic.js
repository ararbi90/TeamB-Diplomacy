...
function calculateConvoy(selectedCountry) {
    $(document).ready(function(){
        ...
        {
            if(adjList[selectedCountry]["adjacencies"].includes(selectedConvoy[ocean].toString()))
            {
            ...
            for(var ocean in selectedConvoy)
            {
                for( var zone in zoneList)
                {
                    if(selectedConvoy[ocean] != zoneList[zone]){
                        if(adjList[selectedConvoy[ocean]]["adjacencies"].includes(zoneList[zone].toString())){
        ...
        {
            adjList[zoneList[ocean]]["adjacencies"].forEach(function(element)
            {
                if(adjList[element]["type"] == "COASTAL"){
                    if(!landingZone.includes(adjList[element]["val"])){
                        landingZone.push(adjList[element]["val"]);
                    }
                }
            });
        }
...
function issueOrder(moveType, origin)
{
    $(document).ready(function(){
        var adjList = {
            "BOH": {"name": "Bohemia", "val": "BOH", "type": "INLAND", "adjacencies": ["GAL", "TYR", "VIE", "MUN", "SIL"]}, 
        ...
        var destination = selector[selector.selectedIndex].value;
